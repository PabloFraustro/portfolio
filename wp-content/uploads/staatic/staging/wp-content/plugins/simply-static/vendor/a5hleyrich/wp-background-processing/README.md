...
<html><body><p># WP Background Processing
...
Queues work on a first in first out basis, which allows additional items to be pushed to the queue even if it&rsquo;s already processing. Saving a new batch of queued items and dispatching while another background processing instance is already running will result in the dispatch shortcutting out and the existing instance eventually picking up the new items and processing them when it is their turn.
